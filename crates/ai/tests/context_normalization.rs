...
//!
//! Tool-result image downgrade rides the same pass; its behaviour and
//! `ImageDowngraded` diagnostics are pinned by `tool_result_images.rs`.
...
async fn openai_historical_user_image_is_replaced_with_the_placeholder() {
    ...
    assert_eq!(
        ...
        json!([
            ...
            { "type": "text", "text": USER_PLACEHOLDER },
        ])
    );
    assert!(
